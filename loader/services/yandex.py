...
from mutagen.id3 import APIC, ID3, TORY, TCON
from mutagen.mp3 import MP3
from pydub import AudioSegment
from yandex_music import Client, Search, Track, Playlist

from loader.models import Song, Author, Album
from loader.tasks import load_ym_file_meta
...
def load_file_meta(track: int):
    ...
    try:
        if sng := Song.objects.filter(name=track.title, album__name=track.albums[0].title):
            return sng.first()
    except IndexError:
        return
...
def load_playlist(link: str):
    ...
    for track in playlist.fetch_tracks():
        load_ym_file_meta.apply_async(kwargs={"track": track.track.id})
